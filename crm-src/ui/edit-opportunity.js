/* Editing the enquiry itself: the details an estimator corrects after the
   ...
   out to be half the story. Kept out of dialogs.js because it is the one form
...
import { dialog, field, text, textarea, number, select, nul, num, toast, fieldError, after } from './form.js';
...
export async function editOpportunity({ opp, me, onDone }) {
  ...
  dialog({
    ...
    body: `
      ...
      <div class="field-row">
        ${field('location', 'Site', text('location', opp.location || ''))}
        ${field('estimated_value', 'Estimated value (USD)', number('estimated_value', opp.estimated_value ?? '', 'min="0" step="100"'))}
      </div>
      <div class="field-row">
    ...
    onSubmit: async (v) => {
      ...
      await api.updateOpportunity(opp.id, {
        ...
        location: nul(v.location), estimated_value: num(v.estimated_value),
